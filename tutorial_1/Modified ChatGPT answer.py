...
def count_valid_states(existing_states, board=None, turn='O'):
    """
    ...
    if board is None: # empty board should be a valid state
        board = [[' ' for _ in range(3)] for _ in range(3)]

    if ''.join([''.join(b) for b in board]) in existing_states: #avoid repetition
        return 0

    if is_valid_state(board):
        valid_states += 1
        existing_states.add(''.join([''.join(b) for b in board]))
    ...
    for r in range(3):
        for c in range(3):
            if board[r][c] == ' ':
                board[r][c] = turn
                valid_states += count_valid_states(existing_states, board, 'X' if turn == 'O' else 'O')
                board[r][c] = ' '
    return valid_states
...
def main():
    ...
    while True:
        ...
        if is_board_full(board):
           ...
           break
    existing_states = set()
    valid_states = count_valid_states(existing_states)
